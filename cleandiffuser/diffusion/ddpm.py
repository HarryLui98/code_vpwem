...
class DDPM(DiffusionModel):
    ...
    def __init__(
        ...
        if isinstance(nn_diffusion, ChiTransformerPTP):
    ...
    def loss(self, x0, condition=None):
        ...
        condition = self.model["condition"](condition) if condition is not None and self.model["condition"] is not None else condition
        ...
        if self.use_condition_cache:
            if len(self.condition_cache_list) == 0:
                ...
                for keys in condition_cfg.keys():
                    condition_cfg_new[keys] = condition_cfg[keys][:, -self.action_steps:, ...]
