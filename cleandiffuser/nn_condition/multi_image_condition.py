...
from robomimic.models.base_nets import ScanCore
...
class MultiImageObsConditionRobomimic(BaseNNCondition):
...
class MultiImageObsConditionMomart(BaseNNCondition):
    ...
    def __init__(self,
        ...
        rgb_model = get_resnet(rgb_model_name)
        # depth_model
        depth_model = get_resnet(rgb_model_name)
        depth_model.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                this_model = ScanCore(input_shape=shape, conv_kwargs=conv_kwargs, 
        ...
        key_transform_map['depth'] = key_transform_map['rgb']
        key_transform_map['depth_wrist'] = key_transform_map['rgb_wrist']
        rgb_keys = sorted(rgb_keys)
        low_dim_keys = sorted(low_dim_keys)
    ...
    def multi_image_forward(self, input_obs_dict):
        batch_size = None
        features = list()
        obs_dict = dict()
        ...
        if self.use_seq:
            # input: (bs, horizon, c, h, w)
            for k in input_obs_dict.keys():
                obs_dict[k] = input_obs_dict[k].flatten(end_dim=1)
        ...
        if self.share_rgb_model:
            ...
            feature = feature.reshape(batch_size,-1)
            features.append(feature)
            # pass all depth obs to depth model
            depths = list()
            ...
            # (N*B,D)
            feature = self.key_model_map['depth'](depths)
            # (N,B,D)
            feature = feature.reshape(-1,batch_size,*feature.shape[1:])
            # (B,N,D)
            feature = torch.moveaxis(feature,0,1)
        ...
        for key in self.scan_keys:
            ...
            if batch_size is None:
                ...
                assert batch_size == scan.shape[0]
            assert scan.shape[1:] == self.key_shape_map[key]
            feature = self.key_model_map[key](scan)
            features.append(feature)
