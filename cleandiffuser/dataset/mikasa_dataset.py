...
class MikasaDataset(BaseDataset):
    """
    Unified MikasaDataset that supports both regular and embedding data.
    Automatically determines data source based on obs_shape_meta:
    - If key ends with 'emb', reads from emb_{i}.npz files
    - Otherwise, reads from train_data_{i}.npz files
    """
...
def _convert_mikasa_to_replay(store, shape_meta, dataset_path,
    ...
    - shape_meta: dict
        ...
        Automatically determines data source based on key names:
        - If key ends with 'emb', reads from emb_{i}.npz files
        - Otherwise, reads from train_data_{i}.npz files
        For example:
        shape_meta = {
            ...
            "obs": {
                ...
                "dp_emb": {"shape": [256, ], "type": "low_dim"},  # Will read from emb_*.npz
                "joints": {"shape": [7, ], "type": "low_dim"},    # Will read from train_data_*.npz
            }}
    - dataset_path: str
    ...
    - max_inflight_tasks: int
        Maximum number of inflight tasks for parallel processing
    """
    ...
    rgb_keys = list()
    lowdim_keys = list()
    # Determine which keys need to be loaded from emb files
    # Keys ending with 'emb' should be loaded from emb_*.npz files
    obs_shape_meta = shape_meta['obs']
    for key, attr in obs_shape_meta.items():
        shape = attr['shape']
        type = attr.get('type', 'low_dim')
    ...
    # Determine number of episodes by checking files
    episode_ends = list()
    prev_end = 0
    max_episodes = 1000
    ...
    for data_key in tqdm(lowdim_keys + ['action'], desc="Loading lowdim data"):
        ...
        if len(this_data) == 0:
            raise ValueError(f"No data loaded for key {data_key}")
                ...
                for key in rgb_keys:
                    ...
                    for episode_idx in range(n_episodes):
                        ...
                        try:
                            ...
                            print(f"Warning: Failed to load {key} from episode {episode_idx}: {e}")
                            continue
                completed, futures = concurrent.futures.wait(futures)
                for f in completed:
                    if not f.result():
