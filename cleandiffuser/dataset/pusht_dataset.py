...
from cleandiffuser.dataset.dataset_utils import SequenceSampler, MinMaxNormalizer, ImageNormalizer, EmptyNormalizer, dict_apply
...
class PushTEmbDataset(BaseDataset):
    def __init__(self,
            zarr_path,
            obs_keys=['mail_emb', 'action'], 
            horizon=1,
            pad_before=0,
            pad_after=0,
            abs_action=False
    ...
    def get_normalizer(self):
        ...
        # image_normalizer = ImageNormalizer()
        action_normalizer = MinMaxNormalizer(self.replay_buffer['action'][:])
        mail_emb_normalizer = EmptyNormalizer()  # do nothing
        
        return {
            "obs": {
                "mail_emb": mail_emb_normalizer,
            },
            "action": action_normalizer
        }
    ...
    def _sample_to_data(self, sample):
        ...
        # dp_emb
        # dp_emb = sample['dp_emb'].astype(np.float32)
        # dp_emb = self.normalizer['obs']['dp_emb'].normalize(dp_emb)

        mail_emb = sample['mail_emb'].astype(np.float32)
        mail_emb = self.normalizer['obs']['mail_emb'].normalize(mail_emb)
