...
class LongMemSequenceDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        hdf5_path,
        obs_keys,
        ...
        Args:
            ...
            memory_type (str): type of memory sampling. Currently not used with subsample_ratio.
            
            subsample_ratio (int): ratio for subsampling memory frames. Samples every N frames from 0 
                to the current index. This means earlier frames have fewer samples, later frames have more.
                Defaults to 10.
        """
        super(LongMemSequenceDataset, self).__init__()
        ...
        # memory related hyperparameters
        self.memory_type = memory_type
        # Use subsample_ratio for dynamic sampling
        # Sample every subsample_ratio frames from 0 to index_in_demo
    ...
    def get_memory_from_demo(self, demo_id, index_in_demo, keys, num_frames_to_stack=0, seq_length=1):
        """
        Extract a (sub)sequence of data items from a demo given the @keys of the items.
        ...
        # and randomly sample one frame from each segment (vectorized)
        if seq_begin_index == 0:
            ...
            # For each segment [seg_start, seg_end), randomly pick one index
            segment_sizes = seg_ends - seg_starts
            # Generate random offsets within each segment
            random_offsets = np.random.randint(0, segment_sizes, size=len(seg_starts))
            # Add offsets to segment starts to get final indices
            selected_frame_idx = (seg_starts + random_offsets).tolist()
        
        # Ensure at least one frame (the first frame)
        if len(selected_frame_idx) == 0:
            selected_frame_idx = [0]
        
        # Store actual number of frames for dynamic batching
        actual_num_frames = len(selected_frame_idx)
        
        for k in keys:
            data = self.get_dataset_for_ep(demo_id, k)
            seq[k] = data[selected_frame_idx].astype("float32")
        
        seq['ep_step'] = np.array(selected_frame_idx)
        seq['actual_num_frames'] = actual_num_frames  # Store for dynamic batching
    ...
    def get_obs_sequence_from_demo(self, demo_id, index_in_demo, keys, num_frames_to_stack=0, seq_length=1, prefix="obs"):
        ...
        all_obs = {k.split('/')[1]: obs[k] for k in obs}  # strip the prefix
        if self.get_pad_mask:
            all_obs["pad_mask"] = pad_mask

        obs_mem = self.get_memory_from_demo(
            demo_id,
            index_in_demo=index_in_demo,
            keys=tuple('{}/{}'.format(prefix, k) for k in keys),
            num_frames_to_stack=num_frames_to_stack,
        ...
        for k in obs:
            all_obs[k.split('/')[1]+'_mem'] = obs_mem[k]
        
        all_obs["ep_step"] = obs_mem["ep_step"]
        if "actual_num_frames" in obs_mem:
            all_obs["actual_num_frames"] = obs_mem["actual_num_frames"]

        # prepare image observations from dataset
        return ObsUtils.process_obs_dict(all_obs)
...
class SortedByLengthSampler(torch.utils.data.Sampler):
    ...
    def __init__(self, dataset, shuffle=True):
        ...
        # Calculate actual_num_frames for each sample
        for idx in tqdm(range(len(dataset)), desc="Computing actual_num_frames for all samples"):
            demo_id = dataset._index_to_demo_id[idx]
            demo_start_index = dataset._demo_id_to_start_indices[demo_id]
            demo_length = dataset._demo_id_to_demo_length[demo_id]
            
            # Calculate index_in_demo (same logic as in get_item)
            ...
            # Calculate seq_begin_index (same logic as in get_memory_from_demo)
            seq_begin_index = max(0, index_in_demo - 0)  # num_frames_to_stack is 0 for memory
            
            # Calculate actual_num_frames using subsample_ratio
            selected_frame_idx = list(range(0, seq_begin_index, dataset.subsample_ratio))
            if len(selected_frame_idx) == 0:
                selected_frame_idx = [0]
            actual_num_frames = len(selected_frame_idx)
            
            self.actual_num_frames_list.append(actual_num_frames)
    ...
    def __iter__(self):
        if self.shuffle:
            ...
            groups = {}
            for sorted_idx in self.sorted_indices:
                # sorted_idx is the position in sorted order, we need the original dataset index
...
def dynamic_batch_collate_fn(batch):
    ...
    max_len = 1
    for sample in batch:
        assert 'actual_num_frames' in sample['obs']
