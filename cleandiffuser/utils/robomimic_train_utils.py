...
from .robomimic_dataset_utils import LongMemSequenceDataset
...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    ...
    ds_kwargs = dict(
        ...
        memory_type=config.memory.dataset_memory_type,
        subsample_ratio=config.memory.dataset_memory_subsample_ratio,
    )
    dataset = LongMemSequenceDataset(**ds_kwargs)
