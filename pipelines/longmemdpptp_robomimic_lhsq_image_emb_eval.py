...
from cleandiffuser.env.robomimic.robomimic_image_wrapper import RobomimicImageWrapper, LongSquareWrapper
...
def inference(args, envs, dataset, agent, logger, env_init_fn_dills):
    ...
    for i in range(args.eval_episodes // args.num_envs): 
        ...
        while t < args.max_episode_steps:
            ...
            ep_reward += reward
            t += args.action_steps
            avg_infer_times.append(time_1 - time_0)
...
@hydra.main(config_path="../configs/dp/robomimic_multi_modal/chi_transformer_ptp_longmem", config_name="longsquare_abs_emb_eval")
def pipeline(args):
    # ---------------- Create Logger ----------------
    set_seed(args.seed)
    ...
    if args.diffusion == "ddpm":
        ...
        action_range = torch.Tensor(dataset.normalizer['action'].range)
        action_min = torch.Tensor(dataset.normalizer['action'].min)
        agent = LongMemDDPM(
    ...
    condition_model_path = "PATH_TO_SHORT_CONTEXT_POLICY_CKPT_MODEL"
