...
@hydra.main(config_path="../configs/dp/momart/chi_transformer_ptp", config_name="momart_emb")
def pipeline(args):
    # ---------------- Create Logger ----------------
    set_seed(args.seed)
    ...
    for g_step in [500000, 400000, 300000, 200000, 100000, 0]:
        last_saved_model_path = args.work_dir +  f"/models/model_{g_step}.pt"
        if os.path.exists(last_saved_model_path):
            agent.load(last_saved_model_path)
            n_gradient_step = g_step + 1
            lr_sd = torch.load(args.work_dir +  f'/models/scheduler_{str(g_step)}.pt', map_location=args.device)
            lr_scheduler = CosineAnnealingLR(agent.optimizer, T_max=args.gradient_steps, last_epoch=lr_sd["last_epoch"])
    ...
    if args.mode == "train":
        ...
        for batch in loop_dataloader(dataloader):
            ...
            naction = batch['actions'].numpy()#.to(args.device)  # (B, Ta, D)
