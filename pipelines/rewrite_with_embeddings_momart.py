...
class HDF5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for key in self.obs_keys:
            ...
            elif 'type' in self.obs_dict[key] and self.obs_dict[key]['type'] == 'depth':
                obs[key] = np.moveaxis(obs[key],-1,1).astype(np.float32)
...
def main(args):
    ...
    last_saved_model_path = f"PATH_TO_CKPT_MODEL"
    if os.path.exists(last_saved_model_path):
        agent.load(last_saved_model_path)
    
    _convert_h5_to_embeddings(f"PATH_TO_DATASET/momart/{args.env_name}_{args.env_type}.hdf5", 
                              agent.model_ema.condition, args.shape_meta.obs, 
                              dataset.normalizer, batch_size=args.batch_size, device="cuda:0")
